...
**核心洞察**: TikTok LIVE 平台已经在跑 **高光识别 + 游戏识别** 管道(主播侧标签 / 推荐 / 内容审核 已经在用)。 Encore 直接消费这两个现成信号, **我们这边不需要再跑一遍 Vision**。 成本路径分两阶段:

**Phase 1 — M9 上线方案(单位成本 ≈ 0)**:

- 输入: 平台已有的 [高光时刻] + [游戏 title 分类] 信号(零增量成本)
- 触发: (高光 + 游戏 X) → 直接匹配跟游戏 title 绑定的预制 Encore 模板
- 核心投入 = **预生成大量游戏模板**(品类 × 主题组合, 一次性投入)
- 每路 Encore 启动的增量 LLM 成本 = **~$0**(模板从预生成池里选, 不是实时 LLM 生成)
- 优势: 上线快、单位成本接近 0、行为可预测、不依赖 LLM 实时延迟
- 劣势: 玩法跟主播这一波具体高光契合度有限(只能按 [游戏 × 大类场景] 匹配粗粒度)

**Phase 2 — M14+ 长期方案(实时生成式)**:

- 同样的输入信号 + Sonnet / Gemini 强模型理解高光的具体玩法 → 实时生成定制 Encore 游戏参数
- 优势: 玩法跟高光紧密契合(1v3 retake / sniper headshot / clutch revive 是 3 个不同子场景), 不再是粗粒度模板
- 劣势: 实时生成的延迟 + 效果质量难保障 —— 高光到 Encore 弹出 < 5s 是硬约束, LLM 挤进这个窗口的靠谱性需要工程验证
- 单条 LLM 成本量级估算: Sonnet ~3K input + ~500 output ≈ **~$0.017 / 高光**, 50 高光/直播·小时 × 50K 同时在播 ≈ **~$30K / 天 = ~$1M / 月**, 量级跟原"自跑 Vision 优化后"路径差不多, 但效果好得多

**为什么 Phase 1 选预制模板而不是直接上 LLM**:

1. 上线速度 — M9 内测 5-10 主播需要稳定流程, LLM 在 5s 窗口靠谱性还没验证
2. 单位成本 — Phase 1 几乎免费, 第二阶段收入流(厂商 CPI / 联运)还没起来时, 不烧 LLM 是负责任的选择
3. 可预测性 — 模板行为可预测, 灰度排查问题更简单

**Render 成本**(两阶段共用, 次要):

- M9 客户端 Canvas + MediaRecorder 端侧合成(PR #24 已跑通)= 零服务端成本
- 生产环境若改服务端重渲: 全量 50K 直播间 × 50 高光 × 3 winner = 7.5M clip/日 × 0.5-1 CPU·s ≈ ~$50-200/日, 可忽略
