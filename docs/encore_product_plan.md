...
|---|---|---|---|
| **互动游戏**(萌宠快跑等新建设)| 化身游戏角色, 被礼物驱动加速 / 撞障碍 | 送指定礼物触发预设特效 | **高**(整个 loop 依赖主播被驱动)|
...
<!-- (旧 §2 "一句话需求" 已合并到 §1 顶部的"一句话总结", 跟 Feishu canonical 结构对齐) -->
...
Result Page:
  ...
  · 非 winner 状态:不弹付费(v0.8.1 砍 Spotlight),改强 Play again CTA
   ↓
高光场结束 (~3-5 min 后异步):
...
| **主播开播要求** | Windows PC + LIVE Studio + 白名单 + 下载游戏 / mod | **零额外动作**, 平台后台开关即可 |
| **推广路径** | 区域 LIVE Ops BD 一个个对接白名单主播 | 平台直接对全量游戏直播开 |
| **直播结束后留下什么** | 直播本身 + 营收数字 | **结构化短视频**(前 3 自动剪上下分屏 .webm 发玩家 TikTok + @ 主播 + #Encore tag)|
| **对 TikTok feed 算法** | 无新内容池 | **#Encore 标签 = 算法可吸收的新内容池** |
| **对游戏发行漏斗** | 无 | 短视频里嵌 "Play [游戏名]" = 天然 install 漏斗 |
| **变现模型** | 单一: 直播礼物 | 三类: 观众付费 + 厂商付费 CPI install + 多人玩社交杠杆 |
| **蚕食游戏直播品类** | **是**(主播变 MC, 核心电竞向主播无法接入) | **否**(主播照常打 ranked, 反而是品类的伴生玩法)|

**关键论点**:

互动游戏 / 弹幕游戏的产品逻辑 = **"主播被观众影响"** —— 整个 loop 依赖主播喊话求礼物。 适合**已是娱乐 / MC 心智**的主播; 但**核心游戏 / 段位 / 电竞向主播 接不进来** —— 这群人的核心 KPI 是打得好, 不是讨礼物。 而这群人 = TikTok Gaming 流量的**大头**(FPS / MOBA / 吃鸡)。

Encore 的产品逻辑 = **"主播无关, 观众自己玩"** —— loop 在主播零参与的前提下也能转。 这让 Encore 能服务核心游戏主播这群被现有礼物互动覆盖不到的人。

**三方互补 —— 主播策略组合的三端**: 同一主播可以同时用两边(打 ranked 用 Encore, 拉氛围用萌宠快跑); 同一观众在两个产品里得到不同满足(送礼影响主播 vs 自己上手玩)。
...
### 7.2 成本模型(三档方案, 全部 ≤ $10 万/月)

**核心洞察**: TikTok LIVE 平台已经在跑 **高光识别 + 游戏识别** 管道(主播侧标签 / 推荐 / 内容审核 已经在用)。 Encore 直接消费现成信号, **我们这边不需要再跑一遍 Vision**。

#### Phase 1 (M9 上线) · 零增量成本

- 输入: 平台已有的 [高光时刻] + [游戏 title 分类] 信号(免费)
- 触发: (高光 + 游戏 X) → 匹配跟游戏 title 绑定的**手工预制模板**
- 核心投入 = 预生成模板(品类 × 主题组合, 一次性投入)
- 每路 Encore 启动的增量 LLM 成本 = **~$0**
- 灰度 / M11 礼物经济期都跑这一套, 不烧钱
- 劣势: 跟主播这一波具体高光契合度有限(粗粒度匹配)

#### Phase 2 (M14+) · 三档方案 from 最便宜到较高, **全部 ≤ $10 万/月**

> 规模假设: 全量 50K 直播间同时在播, 50 高光/直播间·小时, 5h/直播间·日, ~75M 高光/月。

**方案 A · 最便宜 (~$5-15K / 月) · 离线模板工厂**:

- 路径: 每月跑一次 offline batch, LLM 生成 10K-50K 模板变体, 按 (游戏 × 场景类型 × 玩感细节) hash 入库
- Runtime: 检测高光 → 算 hash → 查库 → **99% 命中**, 直接用预生成模板; 1% miss → fall through 到 Haiku 实时生成
- 成本拆解: 离线 batch ~$5K/月 + runtime fall-through 1% × Haiku ~$300/月
- 优势: 成本最低 / runtime 0 LLM 延迟 / batch 时可多 candidate 选最好 / LLM 用更长 prompt 更深思考
- 劣势: 模板池预生成, 跟主播这一波具体细节的契合度有上限
- 适合何时: 第二阶段收入流(厂商 CPI / 联运)还在爬坡

**方案 B · 中等 (~$30-50K / 月) · Haiku 实时 + prompt cache**:

- 路径: 每个高光过 Claude 3 Haiku, system prompt(模板定义 + V2G schema + few-shot)缓存 90%
- 实际 input 等效 ~500 tokens, output ~500 tokens
- 单条: Haiku $0.25/M in + $1.25/M out → ~$0.0005 / 高光
- 全量 75M × $0.0005 ≈ **~$37K / 月**, 加 overhead → 30-50K
- 优势: 每个高光都有 LLM 看一眼, 契合度比模板工厂高一档
- 劣势: 每条都 LLM call, runtime 5s 窗口需 trade-off
- 适合何时: 厂商投放 + 月卡跑通后, 想给观众更好契合度

**方案 C · 较高 (~$75-95K / 月) · 智能分级 (Sonnet for top 10% + Haiku for rest)**:

- 路径: 用 Haiku 给 highlight 打分(ace / clutch / comeback 顶级 vs 普通), top 10% → Sonnet 出最高质量参数, 90% → Haiku
...
- 适合何时: 第二阶段收入稳定后, 追求最高契合度

**为什么 Phase 1 不直接上三档之一**:

1. 上线速度 — M9 内测 5-10 主播需要稳定流程, LLM 5s 窗口靠谱性未验证
2. 单位成本 — Phase 1 几乎免费, 收入流还没起来时不烧 LLM
3. 可预测性 — 手工模板可预测, 灰度排查简单

**Render 成本**(全方案共用, 次要):

- M9 客户端 Canvas + MediaRecorder 端侧合成(PR #24 已跑通)= 零服务端成本
- 生产环境若改服务端重渲: 全量 50K × 50 高光 × 3 winner = 7.5M clip/日 × 0.5-1 CPU·s ≈ ~$50-200/日, 可忽略
...
| **二次分发** | 仅 Top 3 winner 自动 clip 免费(v0.8.1);post 主体永远是 viewer,主播 @tag + 可选 Duet |
| **视觉格式** | 默认上下 split,前后 mirror 为 backup |
| **付费 SKU** | **MVP 仅 Enhance**(v0.8.1 砍 Spotlight);Phase 2 加 Remix;**Sponsor / Loot 不做** |
