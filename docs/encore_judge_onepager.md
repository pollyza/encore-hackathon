...
**成本路径分两阶段**(关键洞察:复用平台已有 高光识别 + 游戏识别 管道, 我们不自跑 Vision):

| 阶段 | 怎么做 | 增量 LLM 成本 |
|---|---|---|
| **阶段 1 · M9 上线** | (高光 + 游戏 X)→ 匹配预制 Encore 模板, 投入是**多预生成模板**(品类 × 主题, 一次性)| **~$0**(单位成本接近 0)|
| **阶段 2 · M14+ 长期** | Sonnet / Gemini 实时理解高光玩法 → 生成定制 Encore 参数(契合度高, 但延迟 + 质量待验证)| ~$0.017/高光 → 全量 50K 同时在播 ≈ **~$1M/月** |
