...
**成本路径**(关键洞察:平台已有 高光识别 + 游戏识别 当过滤层,Encore 只决定"模板还是真生成";模型按日预算自动降级):

> **规模假设**: TikTok 游戏直播大盘 ~4B 高光/月。 **但 Encore 不是每条都跑 LLM** —— 过滤层把它压到很小:
>
> 90 万游戏主播/天 × 50% 值得做 × 每天最多 3 次 × 30 天 = **~4050 万次/月 月候选池**
>
> 这是 Encore 需要决定"走模板还是走真生成"的池子, 不是模型调用量。

| 阶段 | 怎么做 | 月成本封顶 |
|---|---|---|
| **第一阶段 · M9 上线** | 5-20 款模板覆盖 Roblox / Free Fire / GTA / MLBB / PUBG / MC 等主流;不调模型 | **~$0 增量** |
| **第二阶段 · 基础版** | 4050 万候选, 80% 走模板, 20% 真生成 = 810 万次真生成/月。 路由 = 80% GPT-5.4 mini / Claude Haiku + 15% Sonnet + 5% GPT-5.5 / Opus | **~$70-72K / 月** |
| **第二阶段 · 放量版** | 同路由, 真生成比例提到 ~30%。 **日预算闸门**自动降级:用了 >70% 转便宜模型 / >90% 只走模板 | **≤ $100K / 月封顶** |

**$100K/月封顶不是承诺, 是工程保证** — 月预算 ÷ 30 ≈ 日预算 $3,333, 系统按日封顶;预留 $15K/月给模板刷新 + 监控。

> **回本线**:一次高光只生成 1 份玩法参数, 整个房间观众一起玩, 模型成本被全场摊薄。 按 $0.5 Enhance 算:GPT-5.5 ≈ 0.06 笔回本, Sonnet ≈ 0.03 笔, mini / Haiku ≈ 0.01 笔。 贵模型只用在头部房间 / 高传播高光 / 厂商合作游戏。
...
**结论**:Phase 1 **~$1.5M/月** → Phase 2 **~$2M/月** → Phase 3 **~$2.1M/月**。 模型成本 $70-100K/月,**净利率 95%+**。

口径(跟成本测算同口径):月 Encore 触发 **4050 万** × 每次平均 **5 玩家** = **2.025 亿月玩家 session**。

| 阶段 | 启用 SKU | 月净收 | 怎么算 |
|---|---|---|---|
| **Phase 1 放量** (M11-M12) | A only · Enhance | **$1.5M** | 2.025 亿 × 3% 转化 × $0.5 × 50% 分成 |
| **Phase 2 基础版** (M12-M14) | A + B-1 · CPI 漏斗 | **$2.0M** | + 12.15M shorts × 400 vv × 0.2% click × 5% install × $1 CPI = $0.5M |
| **Phase 2 放量版** (M14+) | A + B 全(含联运) | **$2.1M** | + 联运 2 家 × $30K = $60K |
| **Phase 3 长期** (M18+) | A + B + C(含多人玩) | **$2.1M** | + 2 sponsor × $10K = $20K(C 主要拉留存,数字本身小) |

**三类收入对应的人**:
- 🅰 **观众付费**:玩得更爽 → Enhance($0.5 续命,第一版)→ 皮肤 → AI 自定义(Remix)→ 月卡。 经典手游 LTV 阶梯。
- 🅱 **厂商付费**:把这当广告位 → 短视频 "Play [游戏]" CTA 跑 CPI 漏斗 + 品牌联名 Encore + 联运资源位。 **第二阶段长期最大头**。
- 🅲 **多人玩**:社交杠杆 → Encore 联赛 + 集体送礼解锁 + 主播同框打。 主要价值在拉高 A 留存。

**没算的**:国别 ARPU 差异 / 支付通道手续费(~5%)/ 退款(~3%)/ 主播补贴 / 人力成本。 **不做赌博、抽奖、影响主播游戏的礼物**。 测算细节 + 敏感度见飞书 Brief §6.3。
...
- **成本路径**:第一阶段复用平台已有 高光识别 + 游戏识别 当过滤层 + 5-20 套模板 → **单位成本接近 $0**。 第二阶段才上模型,基础版 **$70-72K/月**,放量版按 **日预算闸门** 自动降级、**$100K/月封顶**。 详见上面成本表。
- **法务 / IP**:全程抽象像素 + 程序化美术,**不碰任何游戏 IP**;Vision 只读视觉信号,不 OCR 游戏 UI 文字。
- **二次分发架构**:玩家端只上传按键日志(几 KB),服务端用同样的随机种子重跑游戏并录像 → 全量 7.5M 条/天的渲染成本仅 ~$50-200/日,相比模型成本可忽略。
- **路线图**:
...
| 设计真理协作 | Lark Docs ([Brief 飞书 doc](https://bytedance.my.larkoffice.com/docx/VdRFd5MkBopEjyxAJ70mqmvJyfc) + Demo Dev Plan v0.2) |
