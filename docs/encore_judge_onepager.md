...
**成本路径**(关键洞察:复用平台已有 高光识别 + 游戏识别, 不自跑 Vision):

> **规模假设**: 每日开播主播 ~ 900K, 平均 3h × 50 高光/h ≈ **~4B 高光/月**(全量大盘)。

| 阶段 / 方案 | 怎么做 | 全量月成本 |
|---|---|---|
| **Phase 1 · M9 上线** | 平台信号 → 匹配手工预制模板(粗粒度)| **~$0** 增量 |
| **Phase 2-A · M14+ 最便宜** | 池 20 万模板, 99.95% 命中, 0.05% Haiku 兜底 | **~$15-25K / 月** |
| **Phase 2-B · M14+ 中等** | 池 5 万模板, 99% 命中, 1% Haiku 兜底 | **~$30-50K / 月** |
| **Phase 2-C · M14+ 较高** | 池 10 万 + miss 智能分级(顶级 → Sonnet, 普通 → Haiku) | **~$60-95K / 月** |

**Phase 2 三档全部 ≤ $10 万/月**, 由当时收入流(厂商 CPI / 联运)的成熟度选最匹配的一档。

> 4B/月这个量级下"每条都跑 LLM"已经不现实(单条最便宜 Haiku ~$0.0005, 全量 ≈ $2M/月)。 三档的差异 = 缓存命中率 + miss fallback 策略。
